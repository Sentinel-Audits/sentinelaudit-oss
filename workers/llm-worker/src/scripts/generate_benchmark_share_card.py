...
def main():
    ...
    severity_counts = intelligence.get("severityCounts", {})
    severity_known = sum(int(v or 0) for v in severity_counts.values()) > 0
    severity_summary = " - ".join(
        [
            f"high {int(severity_counts.get('high', 0))}",
            f"medium {int(severity_counts.get('medium', 0))}",
            f"low {int(severity_counts.get('low', 0))}",
            f"info {int(severity_counts.get('info', 0)) + int(severity_counts.get('informational', 0)) + int(severity_counts.get('optimization', 0))}",
        ]
    )
    ...
    draw_metric(
        ...
        (820, 248, 1160, 370),
        "Production Findings",
        str(intelligence["reportFindings"] + intelligence["needsReview"] + intelligence["researchNotes"]),
        f"{intelligence['reportFindings']} report - {intelligence['needsReview']} review - {intelligence['researchNotes']} research",
    )
    draw_metric(
        draw,
        (1190, 248, 1530, 370),
        "Severity Mix",
        str(int(severity_counts.get("high", 0)) + int(severity_counts.get("medium", 0)) + int(severity_counts.get("low", 0)))
        if severity_known
        else "n/a",
        severity_summary if severity_known else "severity counts unavailable in this snapshot",
    )
    ...
    corpus_lines = [f"{fixture['name']} ({fixture['cases']} cases)" for fixture in repo["fixtureBreakdown"][:4]]
    y = 556
    for line in corpus_lines:
        draw.text((110, y), line, fill="#FAFAFA", font=FONT_BODY)
        y += 34
    draw.text((110, 690), "Auditor review focus", fill="#A1A1AA", font=FONT_TEXT)
    focus_lines = [f"{area['tag']} ({area['count']})" for area in auditor["focusAreas"][:3]]
    y = 724
    ...
    draw.text((850, 464), "Production telemetry and method", fill="#FAFAFA", font=FONT_LABEL)
    top_signal = intelligence["topImprovementSignals"][0]["key"] if intelligence["topImprovementSignals"] else "none"
    top_mismatch = intelligence["topDimensionalMismatchKinds"][0]["kind"] if intelligence["topDimensionalMismatchKinds"] else "none"
    telemetry_lines = [
        f"{intelligence['artifacts']} local audit artifacts",
        f"provenance coverage: {pct(intelligence['provenanceCoverage'])}",
        f"value-flow coverage: {pct(intelligence['valueFlowCoverage'])}",
        f"dimensional coverage: {pct(intelligence['dimensionalCoverage'])}",
    ...
    for line in telemetry_lines:
        ...
        y += 24
    draw.text((850, 660), "Methodology", fill="#A1A1AA", font=FONT_TEXT)
    method_lines = [
        "Evidence layers",
